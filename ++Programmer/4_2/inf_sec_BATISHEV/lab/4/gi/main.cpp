...
enum SHA3_FLAGS sha3_SetFlags(void* priv, enum SHA3_FLAGS flags1) {
    sha3_context* ctx = (sha3_context*)priv;
    flags1 = SHA3_FLAGS_KECCAK;
    ctx->capacityWords |=
        (flags1 == SHA3_FLAGS_KECCAK ? SHA3_USE_KECCAK_FLAG : 0);
    return flags1;
}
...
void signing_algo(mpz_t p, mpz_t q, mpz_t g, mpz_t x, mpz_t y, char message[],
                  mpz_t* r, mpz_t* s) {
    cout << "\n---------------------------------------------SIGNING------------"
            "---------------------------------------\n";
    uint8_t buf[32];
    ...
    // Calling SHA3-224
    sha3_HashBuffer(256, SHA3_FLAGS_KECCAK, (void*)message, 3, buf,
                    sizeof(buf));
...
void verification_algo(mpz_t p, mpz_t q, mpz_t g, mpz_t y, char message[],
                       mpz_t r, mpz_t s) {
    cout << "\n---------------------------------------------VERIFICATION-------"
            "--------------------------------------------\n";
    uint8_t buf[32];
    ...
    for (int i = 0; i < 32; i++) {
        ...
        int t = buf[i];
        sprintf(buffer, "%ld",
                t); // Если t — это mpz_t, сначала нужно получить его значение
        strcat(ch, buffer);
    }
    mpz_set_str(Hm, ch, 10);
    gmp_printf("\n H(m) for message(%s) : %Zd", message, Hm);
...
int main() {
    ...
    cout << "enter message to be signed: ";
