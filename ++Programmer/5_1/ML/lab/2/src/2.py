...
dtc = DecisionTreeClassifier(max_depth=3, random_state=random_state, criterion="entropy")
...
svm = SVC(kernel='linear', gamma='auto', random_state=random_state)
...
# 9. Сведите результаты всех исследований в таблицу и сравните результаты
# классификации # на основе оценки точности по тестовому множетсву (fix: train-множеству)
...
# --- GaussianNB ---
var_smoothing_values = [1e-9, 1e-5, 1e-2]
for vs in var_smoothing_values:
    model = GaussianNB(var_smoothing=vs)
    model.fit(X_train, y_train)
    y_pred = model.predict(X_train)  # фикс: считаем на train
...
plt.ylabel("Accuracy")
plt.title("Comparison of classifiers with different parameters (Train set)")
plt.legend()
plt.tight_layout()
plt.show()
